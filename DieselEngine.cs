...
{
    class DieselEngine : Abstract_Motor
    {
        public DieselEngine()
        {
        }
        public DieselEngine(string name, string code, double d_Bore, double piston_Stroke, int numberOfEngineCylinders, double volumeEngine, int number_of_valves_per_cylinder, string piston_engine_layout, string timing_drive_type, double engine_compression_ratio, double poewrEngine)
        {
            try
            {
                ChangeInfa(name, code, d_Bore, piston_Stroke, numberOfEngineCylinders, volumeEngine, number_of_valves_per_cylinder, "diesel", piston_engine_layout, timing_drive_type, engine_compression_ratio, poewrEngine);
            }
        ...
        public static DieselEngine CreateNew()
        {
            string name;
            string code;
            double d_Bore;
            ...
            double powerEngine;
            Console.WriteLine(new string('=', 30) + "Дизельный мотор"+new string('=',30));
            ...
            Console.WriteLine("Введите ход поршня(OHV,OHC,SOHC,DOHC)");
            ...
            Console.WriteLine("Введите тип привода ГРМ");
            ...
            return new DieselEngine(name, code, d_Bore, piston_Stroke, numberOfEngineCylinders, volumeEngine, number_of_valves_per_cylinder,
                piston_engine_layout, timing_drive_type, engine_compression_ratio, powerEngine);
        }
        public static DieselEngine CreateNew(Abstract_Motor[] abstract_Motors)
        {
            string name;
            string code;
            double d_Bore;
            ...
            Console.WriteLine("Введите мощность двигателя");
            powerEngine = double.Parse(Console.ReadLine());
            DieselEngine dieselEngine = new DieselEngine(name, code, d_Bore, piston_Stroke, numberOfEngineCylinders, volumeEngine, number_of_valves_per_cylinder,
                piston_engine_layout, timing_drive_type, engine_compression_ratio, powerEngine);
            for (int i = 0; i < abstract_Motors.Length; i++)
                if (abstract_Motors[i] != null) 
                    abstract_Motors[i] = dieselEngine;
