...
{
    class JetEngine : Abstract_Motor
    {
        public JetEngine()
        {
        }
        public JetEngine(string name, string code, double d_Bore, double piston_Stroke, int numberOfEngineCylinders, double volumeEngine, int number_of_valves_per_cylinder, string piston_engine_layout, string timing_drive_type, double engine_compression_ratio, double poewrEngine)
        {
            try
            {
                ChangeInfa(name, code, d_Bore, piston_Stroke, numberOfEngineCylinders, volumeEngine, number_of_valves_per_cylinder, "jet fuel", piston_engine_layout, timing_drive_type, engine_compression_ratio, poewrEngine);
            }
        ...
        public static JetEngine CreateNew()
        {
            string name;
            string code;
            double d_Bore;
            ...
            double powerEngine;
            Console.WriteLine(new string('=', 30) + "Реактивный двигатель" + new string('=', 30));
            ...
            return new JetEngine(name, code, d_Bore, piston_Stroke, numberOfEngineCylinders, volumeEngine, number_of_valves_per_cylinder,
                    piston_engine_layout, timing_drive_type, engine_compression_ratio, powerEngine);
        }
        public static JetEngine CreateNew(Abstract_Motor[] abstract_Motors)
        {
            string name;
            string code;
            double d_Bore;
            ...
            Console.WriteLine("Введите мощность двигателя");
            powerEngine = double.Parse(Console.ReadLine());
            JetEngine jetEngine = new JetEngine(name, code, d_Bore, piston_Stroke, numberOfEngineCylinders, volumeEngine, number_of_valves_per_cylinder,
                    piston_engine_layout, timing_drive_type, engine_compression_ratio, powerEngine);
            for (int i = 0; i < abstract_Motors.Length; i++)
                if (abstract_Motors[i] != null)
                    abstract_Motors[i] = jetEngine;
            return jetEngine;
        }
    }
}
